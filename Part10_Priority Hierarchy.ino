/*
We'll implement: Dijkstra algorithm (Priority Hierarchy) to solve the "Dining philosophers problem"
Here we'll assign a number to each mutex (chopstick)
Each philsoopher (Task) will pick up the chopstick (mutex) with the lower number (mutex 1).
This solves Deadlock, but not Livelock or Starvation
*/
...
void doTaskA(void *parameters) {
  ...
  while (1) {
    ...
    // Give back mutexes. Order doesn't matter but it's considered good practive to release in reverse order of how you took them
