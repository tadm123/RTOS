...
void IRAM_ATTR onTimer() {  //this is our ISR (we only want to use FromISR functions)

 BaseType_t task_woken = pdFALSE;		//initializing task_woken to type “BaseType_t” and set to pdFALSE. pdFALSE is just RTOS for boolean FALSE

 // Perform action (read from ADC)
 val = analogRead(adc_pin);   //adc_pin was our analog pin, so we read and store value on “val” variable
